...
bool check(int i, int l, int r) {
    ...
    return (std::max(f[x][l], f[x][r - (1 << x) + 1]) <= f[0][i] + k);
}

int binary_search_left(int i, int l, int r) {
    while (r - l > 1) {
        int m = (l + r) >> 1;
...
int binary_serach_right(int i, int l, int r) {
    while (r - l > 1) {
        int m = (l + r) >> 1;
...
signed main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        res += binary_search_left(i, 0, i);
        res += binary_serach_right(i, i, n + 1);
