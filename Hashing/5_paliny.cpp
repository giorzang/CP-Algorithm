...
const int MAXN = 5e4 + 5, base = 31;

int HashS[MAXN], HashT[MAXN], POW[MAXN], n, ans;
...
bool is_palid(int l, int r) {
    return getHash(l, r, HashS) == getHash(n - r + 1, n - l + 1, HashT);
}

void binary_search(int i) {
    int l, r;

    l = 0, r = std::min(i, n - i + 1);
    while (r - l > 1) {
        int m = (l + r) >> 1;
